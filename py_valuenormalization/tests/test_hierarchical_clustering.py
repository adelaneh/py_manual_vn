...
1)Calculate distances between all value pairs and assert for min distance value pair.
2)Create dendrogram for different maxclustsize values and assert the dendrogram value. 
3)For each dendrogram created in previous test, we get the valtoclustidmap based on various thresholds and assert on  the value of valtoclustidmap.
4)Threshold is typically decided after looking at the dendrogram and deciding upto which distance value , we would like to cluster.
5)There is also a direct cluster() test on the values which needs to return expected cluster values.
